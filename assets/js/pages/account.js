...
Pages.define("account", function (global) {
  ...
  /* ---------- what a lawyer offers a trainee ----------
     The form used to be here, which put the prices two clicks away from the
     trainees, the applications and the calendar they govern. It lives on the
     supervision page now, with the rest of it; this is the door, and it says
     what is currently published so the door is worth opening. */
  function offer(u) {
    if (!Session.is("lawyer")) return "";
    var on = u.isMentor || u.supervisesCases;
    var bits = [];
    if (u.isMentor) bits.push(I18N.t("men.fee", { n: I18N.num(u.mentorshipFee || 0) }));
    if (u.supervisesCases) bits.push(I18N.t("sup.fee", { n: I18N.num(u.supervisionFee || 0) }));

    return '<section class="card card--pad" style="margin-top:var(--s-6)" data-offer-door>' +
      '<div class="row between wrap gap-3">' +
        "<div><h2 class=\"subtitle\">" + esc(I18N.t("offer.title")) + "</h2>" +
          '<p class="small muted" style="margin-top:var(--s-2);max-width:60ch">' +
            esc(on ? bits.join(" · ") : I18N.t("offer.notPublished")) + "</p></div>" +
        '<a class="btn btn--outline btn--sm" href="mentorship.html?tab=offer">' +
          Icons.svg("graduation", "icon-sm") + esc(I18N.t("offer.manage")) + "</a>" +
      "</div></section>";
  }
